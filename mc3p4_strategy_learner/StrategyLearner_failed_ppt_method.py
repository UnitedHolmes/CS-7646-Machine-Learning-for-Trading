...
class StrategyLearner(object):
    ...
    def addEvidence(self, symbol = "IBM", \
        ...
        self.learner = ql.QLearner(num_states=9,\
            num_actions = 3, \
            alpha = 0.2, \
            gamma = 0.9, \
...
#        self.X3_min = X3_bbp.min(axis=0).values
        ...
        while (training_day < X2_SMA.shape[0]) & (previous_end_port<2.5*sv): 
            ...
            if not np.isnan(X2_SMA.iloc[training_day].values):
                ...
                if (X2_coeff.iloc[training_day].values>1.05) and (X3_bbp.iloc[training_day].values>1):
                    state_indicator = 0
                elif (X2_coeff.iloc[training_day].values<0.95) and (X3_bbp.iloc[training_day].values<0):
                    state_indicator = 2
                else:
                    state_indicator = 1
...
#                    print 'error! Too many states!'
                ...
                state = state_0 * 3 + state_indicator
                if flag_0 == True:
                    flag_0 = False
                ...
                if state_0 == 0:
                    if state_indicator == 0:
                        r = -10
                    elif state_indicator == 1:
                        r = -1
                    elif state_indicator == 2:
                        r = 10
                elif state_0 == 1:
                    if state_indicator == 0:
                        r = 5
...
#        my_action = 0
        ...
        while testing_day < trades.shape[0]:
            ...
            if not np.isnan(X2_SMA.iloc[testing_day].values):
                ...
                state_0 = int(position/200 + 1)
                if (X2_coeff.iloc[testing_day].values>1.05) and (X3_bbp.iloc[testing_day].values>1):
                    state_indicator = 0
                elif (X2_coeff.iloc[testing_day].values<0.95) and (X3_bbp.iloc[testing_day].values<0):
                    state_indicator = 2
                else:
                    state_indicator = 1
